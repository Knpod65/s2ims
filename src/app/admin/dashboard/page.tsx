  ...
  return (
    <AppShell requiredRole="admin">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        ...
        <StatCard value={mockAuditLogs.length} label={lang==='th'?'เหตุการณ์ล่าสุด':'Audit Events'} icon={<Shield size={16}/>} color="text-brand"/>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4"><h3 className="font-semibold text-sm text-ink-1">{lang==='th'?'ผู้ใช้งานตามบทบาท':'Users by Role'}</h3><Link href="/admin/users" className="text-xs text-brand">{lang==='th'?'จัดการ':'Manage'} →</Link></div>
          {[['นักศึกษา','Student','1'],['เจ้าหน้าที่','Staff','1'],['หัวหน้า ESQ','ESQ Head','1'],['ผู้ให้ทุน','Provider','1'],['ผู้ดูแลระบบ','Admin','1']].map(([th,en,count]) => (
            <div key={th} className="flex items-center justify-between py-2 border-b border-white/[0.04] last:border-0">
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4"><h3 className="font-semibold text-sm text-ink-1">{lang==='th'?'เหตุการณ์ล่าสุด':'Recent Audit Events'}</h3><Link href="/admin/audit-log" className="text-xs text-brand">{lang==='th'?'ดูทั้งหมด':'View all'} →</Link></div>
          <div className="space-y-2">
            {mockAuditLogs.slice(0,5).map(log => (
              <div key={log.id} className="text-xs p-2 card-sm">
                <div className="flex items-center gap-2 flex-wrap"><span className="font-mono text-brand">{log.action}</span><span className="text-ink-3">·</span><span className="text-ink-3">{log.actor_name}</span></div>
